...
const fetchPhoneLocation = async () => {
  if (!phoneNumber) {
    alert("Please enter a phone number to track. Example +234 phone number");
    return;
  }

  try {
    const response = await fetch("http://localhost:5000/get-cell-info", {
      method: "POST",
    ...
    if (data.latitude && data.longitude) {
      ...
      alert(`Location Found: Lat ${data.latitude}, Lon ${data.longitude}`);
    } else {
